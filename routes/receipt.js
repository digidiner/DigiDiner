...
router.get('/:id', utils.asyncHandler(async function (req, res, next) {
    ...
    if (await order.load()) {
        ...
        res.status(200).render('receipt', { subtotal: calculateSubtotal(orderItems, menuItems), taxes: calculateTaxes(orderItems, menuItems), total: calculateTotal(orderItems, menuItems, updatedTotal), order: order, orderItems: orderItems, menuItems: menuItems, paymentMethod: paymentMethod }); // Pass the orderData to the receipt.ejs template
    } else {
        next(); // Let it 404
    }
}));
